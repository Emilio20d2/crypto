...
function validateCommon(policy: AutomatedOperationPolicy, context: AutomationMarketContext, proposal: AutomatedOperationProposal): void {
  ...
  if (!(context.currentPriceEur != null && context.currentPriceEur > 0)) proposal.blockers.push("Precio actual no disponible");
  ...
  if (context.confidence < policy.minConfidence) {
    proposal.blockers.push(`Confianza insuficiente: ${context.confidence.toFixed(0)} < ${policy.minConfidence.toFixed(0)}`);
  }
  if (context.independentSourceCount < policy.minIndependentSources) {
    proposal.blockers.push(`Fuentes independientes insuficientes: ${context.independentSourceCount}/${policy.minIndependentSources}`);
  }
  if (context.newestMarketDataAt == null) {
    proposal.blockers.push("No existe fecha verificable del dato de mercado");
  } else {
    const ageMinutes = Math.max(0, (context.evaluatedAt - context.newestMarketDataAt) / 60_000);
    if (ageMinutes > policy.maxMarketDataAgeMinutes) proposal.blockers.push(`Datos demasiado antiguos: ${ageMinutes.toFixed(0)} minutos`);
  }
  if (context.missingSignals.length > 0) proposal.reasons.push(`Señales ausentes declaradas: ${context.missingSignals.join(", ")}`);

  if (!policy.authorization.enabled) proposal.blockers.push("Autorización de automatización desactivada");
  if (policy.authorization.expiresAt != null && context.evaluatedAt > policy.authorization.expiresAt) {
    proposal.blockers.push("Autorización de automatización caducada");
  }
  if (context.executionsToday >= policy.authorization.maxDailyOperations) proposal.blockers.push("Límite diario de operaciones alcanzado");
  if (context.notionalExecutedTodayEur >= policy.authorization.maxDailyNotionalEur) proposal.blockers.push("Límite diario de capital alcanzado");
...
function finalizeState(policy: AutomatedOperationPolicy, proposal: AutomatedOperationProposal): AutomatedOperationProposal {
  if (proposal.blockers.length > 0) {
    const dataBlocked = proposal.blockers.some((reason) =>
      reason.includes("Datos") || reason.includes("Precio") || reason.includes("Fuentes") || reason.includes("Confianza") || reason.includes("fecha verificable"),
    );
...
function evaluateBullSale(policy: AutomatedOperationPolicy, context: AutomationMarketContext): AutomatedOperationProposal {
  ...
  if (gainPct == null || gainPct < config.minimumUnrealizedGainPct) {
    proposal.blockers.push(`Plusvalía insuficiente para vender: ${gainPct == null ? "sin base de coste" : `${gainPct.toFixed(1)}%`}`);
  }
  if (context.sentimentScore == null || context.sentimentScore < config.minimumSentimentScore) {
    proposal.blockers.push("El sentimiento no confirma una fase alcista de recogida");
  }
  if (!(context.assetUnits > 0 && context.assetMarketValueEur > 0)) proposal.blockers.push("No existe posición vendible");
  ...
  if (gainPct != null) proposal.reasons.push(`Plusvalía no realizada ${gainPct.toFixed(1)}%`);
...
function evaluateBearRebuy(policy: AutomatedOperationPolicy, context: AutomationMarketContext): AutomatedOperationProposal {
  ...
  if (context.sentimentScore == null || context.sentimentScore > config.maximumSentimentScore) {
    proposal.blockers.push("El sentimiento todavía no confirma una zona bajista de recompra");
  }
  if (context.stabilizationScore == null || context.stabilizationScore < config.minimumStabilizationScore) {
    proposal.blockers.push("No existe estabilización suficiente para desplegar el siguiente tramo");
  }

  const freeEurc = Math.max(0, context.operatingEurcEur - context.fiscalReserveEur);
  ...
  proposal.amountEur = roundMoney(Math.max(0, amountEur));
  proposal.baseUnits = context.currentPriceEur && context.currentPriceEur > 0 ? roundUnits(proposal.amountEur / context.currentPriceEur) : null;
  proposal.reasons.push(`Régimen ${context.regime}; recompra escalonada del ${percentage.toFixed(1)}% del EURC libre`);
...
export function isProposalExecutable(proposal: AutomatedOperationProposal): boolean {
  return proposal.state === "READY_TO_PREVIEW" && proposal.amountEur > 0 && proposal.blockers.length === 0;
}
