...
| FASE 2 - Modelo de dominio y migraciones de base de datos | VALIDATED | FASE 1 VALIDATED | Dominio V5 estable, tablas versionadas, migracion repetible | `packages/portfolio/src/perspectives-v5/domain/types.ts`, `packages/database/drizzle/0020_perspectives_v5_operations.sql`, `packages/database/drizzle/meta/_journal.json`, `packages/database/src/migration.test.ts` | `0020_perspectives_v5_operations.sql` crea operaciones programadas V5, reservas, previews, ordenes, fills, autorizaciones y modo global `REVIEW_ONLY` | Migracion base vacia/repetida, typecheck y build afectados | `npm --prefix packages/database run test -- src/migration.test.ts`; `npm --prefix packages/portfolio run typecheck`; `npm --prefix packages/market-data run build`; `npm --prefix packages/database run typecheck`; `npm --prefix packages/database run build` | Pendiente | Push/comentario issue pendientes |
| FASE 3 - Motor de fuentes y minimo de 15 fuentes independientes por activo | VALIDATED | FASE 2 VALIDATED | Fuentes BTC/ETH/SUI, corto/medio/largo, deduplicacion y estados VALID/PARTIAL/BLOCKED | `packages/portfolio/src/perspectives-v5/data/source-catalog.ts`, `packages/portfolio/src/perspectives-v5/source-catalog.test.ts`, `packages/portfolio/src/perspectives-v5/index.ts` | Ninguna | Catalogo con 15 fuentes independientes por BTC/ETH/SUI y minimo 5 short/5 medium/5 long por activo | `npm --prefix packages/portfolio run test -- src/perspectives-v5/source-catalog.test.ts`; `npm --prefix packages/portfolio run typecheck`; `npm --prefix packages/portfolio run build` | Pendiente | Fuentes registradas, no activadas; observaciones/consenso quedan para Fase 4 |
