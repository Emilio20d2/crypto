...
beforeEach(() => {
  ...
  (window as any).cryptoControl = {
    ...
    investmentCycles: {
      list: () => ok([{ id: "cycle-1", planId: "plan-1", name: "Ciclo 2026-2030", startDate: now, endDate: null, monthlyAmountEur: 100, contributionCurrency: "EUR", status: "planned", priority: 1, notes: null, createdAt: now, updatedAt: now }]),
      getCurrent: async () => ({ ok: true as const, data: null }),
      create: () => ok({ id: "cycle-new" }),
      update: (_id: string, data: any) => ok({ id: "cycle-1", planId: "plan-1", name: data.name ?? "Ciclo", startDate: data.startDate ?? now, endDate: data.endDate ?? null, monthlyAmountEur: data.monthlyAmountEur ?? 100, contributionCurrency: data.contributionCurrency ?? "EUR", status: data.status ?? "planned", priority: data.priority ?? 1, notes: data.notes ?? null, createdAt: now, updatedAt: now }),
      delete: () => ok(null),
    },
    investmentAssets: {
